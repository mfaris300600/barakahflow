...
import { HIJRI_MONTHS, MAX_HIJRI_YEAR, MIN_HIJRI_YEAR } from '~/lib/hijri';
...
class Configuration extends React.PureComponent {
	...
	handlePdfWorkerMessage = ( { data } ) => {
		if ( data.error ) {
			...
			console.error( '[pdf worker]', data.error, data.stack );
					...
					<Accordion.Item eventKey="general">
						...
						<Accordion.Body>
							...
							<Form.Group controlId="year">
								<Form.Label>Hijri year (AH)</Form.Label>
								<Form.Control
									type="number"
									min={ MIN_HIJRI_YEAR }
									max={ MAX_HIJRI_YEAR }
								...
								/>
								<Form.Text className="text-muted">
									Umm al-Qura range: {MIN_HIJRI_YEAR}–{MAX_HIJRI_YEAR} AH
								</Form.Text>
							</Form.Group>
